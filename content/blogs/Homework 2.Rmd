...
tidyweather <- weather1 %>% 
  pivot_longer(weather1, cols = 2:13, names_to = "Month", values_to = "Delta") %>% 
  #weather1[order([,1], weather1[,2])] arrange month wise desc
  arrange(desc(Year))
...
```{r scatter_plot}
# Put eval = FALSE to knit

tidyweather <- tidyweather %>%
  mutate(date = ymd(paste(as.character(Year), Month, "1")),
         month = month(date), #removed label=TRUE
         year = year(date))
